...
export default function App() {
  ...
  // Only saves if user is authorised to write
    ...
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", background: C.bg, minHeight: "100vh", color: C.text }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              {user ? (
                <div style={{ display: "flex", gap: 8, alignItems: "center", marginLeft: 8 }}>
                  {isAdmin && (
                    <button onClick={() => setShowAdmin(true)} style={{ background: "#7C3AED", color: "white", border: "none", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: "DM Sans, sans-serif" }}>👤 Admin</button>
                  )}
                  ...
                  <button onClick={signOut} style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", color: "white", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontSize: 12, fontFamily: "DM Sans, sans-serif" }}>Sortir</button>
                </div>
              ) : (
                <button onClick={() => setShowAuth(true)} style={{ background: "rgba(255,255,255,0.2)", border: "1.5px solid rgba(255,255,255,0.5)", color: "white", borderRadius: 8, padding: "7px 16px", cursor: "pointer", fontSize: 13, fontWeight: 700, fontFamily: "DM Sans, sans-serif", marginLeft: 8 }}>🔐 Iniciar sessió</button>
              )}
            </div>
          </div>
      ...
      {!canWrite && (
        <div style={{ background: "#FFFBEB", borderBottom: "1px solid #FDE68A", padding: "8px 20px", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, fontSize: 13, color: "#92400E" }}>
          <span>👁️ Mode lectura — el contingut és públic.</span>
          <button onClick={() => setShowAuth(true)} style={{ background: "none", border: "none", color: C.erasmus, cursor: "pointer", fontWeight: 700, fontSize: 13, fontFamily: "DM Sans, sans-serif", textDecoration: "underline" }}>
            {user ? "Accés pendent d'aprovació" : "Inicia sessió per editar"}
          </button>
        </div>
      )}
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "28px 20px" }}>
        {activeTab === 0 && <HomeTab />}
        {activeTab === 1 && <CRMTab contacts={contacts} setContacts={save("sf2-contacts", setContacts)} canWrite={canWrite} />}
        {activeTab === 2 && <CalendarTab boardNotes={boardNotes} setBoard={save("sf2-board", setBoardNotes)} canWrite={canWrite} />}
        {activeTab === 3 && <SFInfoTab />}
        {activeTab === 4 && <BudgetTab expenses={expenses} setExpenses={save("sf2-expenses", setExpenses)} canWrite={canWrite} />}
        {activeTab === 5 && <DiaryTab entries={diaryEntries} setEntries={save("sf2-diary", setDiaryEntries)} canWrite={canWrite} />}
...
// ─── CRM ─────────────────────────────────────────────────────
function CRMTab({ contacts, setContacts, canWrite }) {
...
// ─── CALENDAR ────────────────────────────────────────────────
function CalendarTab({ boardNotes, setBoard, canWrite }) {
...
// ─── DIARY ───────────────────────────────────────────────────
function DiaryTab({ entries, setEntries, canWrite }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
        ...
        {canWrite && <button className="btn" onClick={() => setOpen(!open)}>+ Nova entrada</button>}
      </div>
